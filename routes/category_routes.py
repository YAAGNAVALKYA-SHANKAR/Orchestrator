...
@router.post("/")
async def create_category(category: CategoryBase):return await service.create(data=category)
...
async def bulk_import_categories(overwrite:bool,files:list[UploadFile]=File(...)):return await service.bulk_upload(overwrite,files)
@router.post("/export/")
