...
class AttributeServices:
    ...
    async def create(data):
        ...
        data_dict["attribute_id"]=f"ATTR_{counter_value:04d}"
        existing_doc=await attributes.find_one({"attribute_name":data_dict["attribute_name"]})
        if existing_doc:raise HTTPException(status_code=400,detail=f"Attribute {data_dict["attribute_name"]} already exists!")        
        try:
            result=await attributes.insert_one(data_dict)
    ...
    async def bulk_upload(overwrite:bool,files:list[UploadFile]=File(...)):
        results=[],skipped_count=0,overwritten_count=0,inserted_count=0        
        for file in files:
            ...
            try:
                ...
                if not data:raise HTTPException(status_code=404,detail=f"No Attributes found!")
                new_entries=[]               
                for entry in data:
